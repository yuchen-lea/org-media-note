...
现在，你可以从 =org-media-note-pretty-hydra/body= 或 =org-media-note-transient=（默认）  访问所有功能。
...
所有功能都可由 =org-media-note-pretty-hydra/body= 或 =org-media-note-transient= 触达（取决于 =org-media-note-interface= 变量的设置）：
