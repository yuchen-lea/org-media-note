...
2. 配置
 ...
 #+END_SRC
3. 现在，你可以从 =org-media-note-show-interface= 访问所有功能。
...
所有功能都可从 =org-media-note-show-interface= 访问所有功能。为了适应更紧凑的布局，功能设置等命令进行了语义化分组：

[[../demo/org-media-note-transient.png?raw=true]]

出现位置由 =transient-display-buffer-action= 设置 
...
- Note：插入笔记
  ...
  + =M= 删除选中区域内的所有 org-media-note 链接（=video=、=audio=、=videocite=、=audiocite=）。（=org-media-note-remove-timestamps-in-region=）
...
[[../demo/org-media-note-transient-import.png?raw=true]]
...
ℹ️️️️ 末尾括号为调用的函数，没有标注的功能多为直接调用mpv命令

* 🛠️ 依赖
- [[https://github.com/kljohann/mpv.el][mpv.el]] 媒体的播放与功能接口
- 如果要配合 bib 文件使用，需要安装 bibtex-completion
** 🌐 在线视频播放
